        	...
        	<?php foreach($factory->getModelFactory()->getAttributes() as $name=>$typeFactory): if((strpos($name, 'privacy_policy') !== false) || in_array($name, [])) continue; ?>
                ...
                <?php
                    if(strpos($name, 'file') === false) {
                        ...
                        $fileName = $typeFactory->getModel()->format($feedback->$name);
                        $fileArr = unserialize(htmlspecialchars_decode($fileName));
                        if($fileArr) {
                            foreach ($fileArr as $file) {
                                $path = '/files/feedback/' . $file;
                                echo CHtml::link($file, $path, array('class'=>'', 'download' => $file));
                                echo ' | ';
                            }
                        } else {
                            $path = '/files/feedback/' . $fileName;
                            echo CHtml::link($fileName, $path, array('class'=>'', 'download' => $fileName));
                        }
                    }
                ?><br />
        	<?php endforeach; ?>       		
